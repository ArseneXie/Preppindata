...
customer = sales[['Customer ID', 'Customer Name', 'Year']].drop_duplicates()
customer['Order?'] = 1
...
customer = customer.pivot_table(index=['Customer ID', 'Customer Name', 'First Purchase'],
                                columns='Year', values='Order?', aggfunc=max).reset_index()
...
customer['OrderString'] = customer['OrderString'].apply(lambda x: 
                                                        re.sub('0', 'S', re.sub('(?<=0)(1)', 'R', re.sub('(?<!0)(1)', 'C', re.sub('(^1)', 'N', str(x))))).rjust(4, '-'))
