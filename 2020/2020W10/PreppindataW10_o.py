...
candidate['Pass'] = candidate.apply(lambda x: not((x['Title'][0:1]!=x['LastName'][0:1] and x['Priority']==1) or 
                                                  (x['Title'][0:1]==x['LastName'][0:1] and x['Priority']!=1)), axis=1)
...
candidate['Pass'] = candidate.apply(lambda x: not((x['Title']=='Doctor' and x['LastName']=='Dimmadome') or
...
final = pd.merge(candidate.query('`Priority`==1'),candidate.query('`Priority`==2'),how='inner',on='Pass',suffixes=('_1','_2'))
final[]
